...
很多人写 `for arg in $@` 或 `wrapper() { $*; }`，参数带空格时全崩。

Bash 有四种展开方式，本质区别在于**是否分词**和**是否合并**：
...
wrapper() {
    ...
    # 好的
    "$@"                  # 逐参数透传，空格/通配符/空字符串全部保留
}

wrapper echo "hello world" "foo bar"
# $*       → echo hello world foo bar    (5个参数)
# "$@"     → echo "hello world" "foo bar"  (3个参数，正确)

# 遍历参数
for arg in "$@"; do       # 正确，逐个处理
    echo "$arg"
done

for arg in $@; do         # 错误：hello world 被拆成 hello 和 world
    echo "$arg"
done
```
...
for f in ${files[@]}; do ...     # 错误：空格分裂
```

记住：**所有变量默认加双引号，除非你明确知道需要分词**。
